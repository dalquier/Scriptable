...
def run_item(item: LauncherItem) -> RunnerResult:
    ...
    try:
        root, target = resolve_target(item)
        os.chdir(root)
        sys.argv = [str(target)]
        ...
        globals_dict = {"__name__": "__main__", "__file__": str(target), "__package__": None, "__cached__": None}
        with contextlib.redirect_stdout(capture), contextlib.redirect_stderr(capture):
            exec(code, globals_dict, globals_dict)
        result = RunnerResult(True, time.perf_counter() - started, capture.getvalue(), None)
        item.last_status = "success"
        item.last_error = None
    except BaseException:
        error = traceback.format_exc()
        result = RunnerResult(False, time.perf_counter() - started, capture.getvalue(), error)
