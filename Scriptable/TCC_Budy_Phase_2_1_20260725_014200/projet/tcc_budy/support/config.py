...
DEFAULT_SYSTEM_INSTRUCTIONS = (
    ...
    "immédiat plausible, tu encourages à contacter les secours ou un professionnel humain adapté."
)
...
def load_settings(root: Path) -> Settings:
    ...
    api_key = str(secret_data.get("OPENAI_API_KEY") or secret_data.get("api_key") or "").strip()
    secret_model = str(secret_data.get("OPENAI_MODEL") or "").strip()
    if secret_model:
        data["model"] = secret_model
    requested = str(data.get("provider", "auto")).strip().lower()
    if requested not in {"auto", "simulator", "openai"}:
        raise ConfigurationError("provider doit valoir auto, simulator ou openai.")
    provider = ("openai" if api_key else "simulator") if requested == "auto" else requested
    if provider == "openai" and not api_key:
        raise ConfigurationError("OpenAI est activé mais aucune clé n'est présente dans secrets.json.")
    return Settings(
        provider=provider,
        ...
        system_instructions=str(data.get("system_instructions", DEFAULT_SYSTEM_INSTRUCTIONS)).strip(),
        api_key=api_key,
    )
