"""Persistance SQLite robuste pour AssistantIA corrigé."""
...
class StorageError(RuntimeError):
    """Erreur de stockage lisible par l'application."""
...
class SQLiteStorage:
    ...
    def add_message(self, conversation_id: str, role: str, content: str) -> Dict[str, Any]:
        ...
        if normalized_role not in {"system", "developer", "user", "assistant", "tool"}:
            raise StorageError(f"Rôle non pris en charge : {normalized_role}")
        now = self._now()
        with self._lock, self._connection() as connection:
            cursor = connection.execute(
                "INSERT INTO messages(conversation_id,role,content,created_at) VALUES(?,?,?,?)",
                (conversation_id, normalized_role, str(content), now),
            )
            connection.execute(
    ...
    def close(self) -> None:
        """Conservé pour simplifier le cycle de vie de l'application."""
        return None
