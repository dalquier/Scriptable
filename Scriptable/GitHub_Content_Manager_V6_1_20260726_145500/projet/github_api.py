...
class GitHubClient:
    ...
    def _request(self, method, path, payload=None):
        ...
        headers = {
            ...
            "User-Agent": "GitHub-Content-Manager-Pyto/6.1",
        ...
        try:
            ...
            if exc.code in (401, 403) and not self.token:
                detail = "Cette opération nécessite un jeton GitHub avec Contents: Read and write."
    ...
    def delete_file(self, path, sha, message):
        if not self.token:
            raise GitHubAPIError("Un jeton GitHub est requis pour supprimer un fichier.")
        quoted = urllib.parse.quote(path.strip("/"), safe="/")
        payload = {"message": message, "sha": sha, "branch": self.branch}
        return self._request("DELETE", "contents/{}".format(quoted), payload)

    def create_folder(self, path):
        marker = path.strip("/") + "/.gitkeep"
        return self.put_file(marker, "", "Create folder {}".format(path))
