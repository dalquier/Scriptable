...
class StorageTests(unittest.TestCase):
    ...
    def test_create_add_order_restore_and_new(self):
        ...
        self.assertEqual(second, self.storage.get_latest_conversation_id())
    ...
    def test_management_validation_and_cascade(self):
        ...
        with self.assertRaises(ValueError): self.storage.add_message(cid, "system", "non")
...
class ParserTests(unittest.TestCase):
    ...
    def test_nested_output_text_and_sources(self):
        raw = json.dumps({"output": [{"type": "message", "content": [{"type": "output_text", "text": "A", "annotations": [{"type": "url_citation", "url": "https://example.com/a", "title": "Source"}]}]}]}).encode()
        result = parse_response(raw)
        self.assertEqual("A", result["text"])
        self.assertEqual("example.com", result["sources"][0]["domain"])
    def test_no_text(self):
        with self.assertRaises(OpenAIClientError): _extract_output_text({"output": []})
    def test_invalid_json(self):
        with self.assertRaises(OpenAIClientError): parse_response(b"not-json")
...
class ClientTests(unittest.TestCase):
    def test_payload_web_toggle(self):
        ...
        self.assertEqual(messages, on["input"])
        self.assertIn("instructions", on)
    ...
    def test_http_network_and_timeout_errors(self):
        ...
        for error in cases:
            def opener(*args, _error=error, **kwargs): raise _error
            with self.subTest(error=type(error).__name__):
                with self.assertRaises(OpenAIClientError): OpenAIClient("sk-test", opener=opener).create_response([])
...
class AppTests(unittest.TestCase):
    ...
    def test_empty_refused_and_success_saved_once(self):
        ...
        with self.assertRaises(ValueError): app.send_message("  ")
        ...
        self.assertEqual(1, len([m for m in app.get_messages() if m["content"] == "Question"]))

    def test_error_keeps_user_without_fake_assistant(self):
        class FailingClient:
            def create_response(self, *args, **kwargs): raise OpenAIClientError("échec")
        app = AssistantIAApp(self.storage, FailingClient())
        with self.assertRaises(OpenAIClientError): app.send_message("À réessayer")
