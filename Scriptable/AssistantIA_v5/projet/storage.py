...
class Storage:
    ...
    def create_conversation(self, title: str = "Nouvelle conversation") -> int:
        ...
        now = self._now()
        with self._lock, self._connect() as connection:
            cursor = connection.execute("INSERT INTO conversations(title, created_at, updated_at) VALUES (?, ?, ?)", (clean, now, now))
    ...
    def add_message(self, conversation_id: int, role: str, content: str) -> int:
        ...
        try:
            with self._lock, self._connect() as connection:
                cursor = connection.execute("INSERT INTO messages(conversation_id, role, content, created_at) VALUES (?, ?, ?, ?)", (conversation_id, role, clean, now))
                updated = connection.execute("UPDATE conversations SET updated_at = ? WHERE id = ?", (now, conversation_id))
    ...
    def get_messages(self, conversation_id: int) -> List[Dict[str, Any]]:
        with self._lock, self._connect() as connection:
            rows = connection.execute("SELECT id, role, content, created_at FROM messages WHERE conversation_id = ? ORDER BY id ASC", (conversation_id,)).fetchall()
    ...
    def list_conversations(self) -> List[Dict[str, Any]]:
        with self._lock, self._connect() as connection:
            rows = connection.execute("SELECT * FROM conversations ORDER BY updated_at DESC, id DESC").fetchall()
        return [dict(row) for row in rows]
    ...
    def rename_conversation(self, conversation_id: int, title: str) -> None:
        ...
        with self._lock, self._connect() as connection:
            if connection.execute("UPDATE conversations SET title = ?, updated_at = ? WHERE id = ?", (clean, self._now(), conversation_id)).rowcount != 1:
                raise ValueError("Conversation introuvable.")
    ...
    def close(self) -> None:
        """Aucune connexion persistante: méthode fournie pour un arrêt uniforme."""
