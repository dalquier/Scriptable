...
class ConversationRepository:
    ...
    def append_message(
        ...
        with self.database.transaction() as conn:
            ...
            conn.execute(
                "INSERT INTO messages(id, conversation_id, sequence, role, content, status, request_id, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    message_id,
                    conversation_id,
    ...
    def get_message_by_request_id(self, request_id: str):
        if not request_id:
            return None
        with self.database.read() as conn:
            row = conn.execute(
    ...
    def get_next_message(self, conversation_id: str, sequence: int):
        with self.database.read() as conn:
            row = conn.execute(
    ...
    def update_title_if_default(self, conversation_id: str, text: str):
        title = " ".join(text.strip().split())[:48] or "Nouvelle conversation"
        with self.database.transaction() as conn:
            conn.execute(
                "UPDATE conversations SET title=? "
                "WHERE id=? AND title='Nouvelle conversation'",
