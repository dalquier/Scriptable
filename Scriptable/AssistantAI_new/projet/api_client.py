...
def parse_response(raw: bytes) -> Dict[str, Any]:
    ...
    return {
        ...
        "metadata": {"id": data.get("id"), "status": data.get("status")},
    }
...
class OpenAIClient:
    ...
    def create_response(self, messages: List[Dict[str, str]], enable_web_search: bool = False) -> Dict[str, Any]:
        request = urllib.request.Request(
            config.API_URL,
            data=json.dumps(self.build_payload(messages, enable_web_search)).encode("utf-8"),
            headers={
        ...
        try:
            ...
            details = {
                400: "Requête refusée : vérifiez le modèle et les options.",
                401: "Clé API refusée.",
                403: "Accès au modèle refusé.",
                404: "Modèle ou endpoint introuvable.",
                429: "Quota ou limite de débit atteint.",
            }
            raise OpenAIClientError(
                "Erreur OpenAI HTTP {0} — {1}".format(exc.code, details.get(exc.code, "La requête OpenAI a échoué."))
            ) from exc
