    ...
    def analyze(self, sender=None):
        ...
        def worker():
            try:
                ...
                time.sleep(1.3)
                self.set_status("Extraction structurée de la réponse…")
                data = parse(self.web.evaluate_js(EXTRACT_JS))
                self.web.load_html(build(
                    data.get("question", ""),
                ...
                self.set_status(
                    "Terminé — méthode " + str(debug.get("method", "?"))
                    + ", réponse " + str(debug.get("answerLength", "?"))
                    + " caractères."
                )
            except Exception as exc:
                self.set_status("Analyse impossible : " + str(exc)[:160])
