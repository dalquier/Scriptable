# -*- coding: utf-8 -*-
"""Extraction Codex v8.5 : pouces ou compositeur comme borne de fin."""
...
(function(){
  ...
  const composer = [...document.querySelectorAll('textarea,input,[contenteditable="true"],[role="textbox"]')]
    .find(el => /demander des modifications|poser une question/i.test(
      (el.getAttribute('placeholder') || '') + ' ' + (el.getAttribute('aria-label') || '')
    ));
  if(composer){
    try { composer.scrollIntoView({block:'end'}); } catch(_) {}
  }
...
(function(){
    ...
    for(const el of controls){
      ...
      if(/demander des modifications|poser une question|ask for changes/i.test(label)){
        ...
        while(node && node !== document.body){
          const rect = node.getBoundingClientRect();
          const t = text(node);
          if(rect.width > 250 && rect.height > 45 && rect.height < 260 &&
             /demander des modifications|poser une question|ask for changes/i.test(t + ' ' + label)){
            return node;
          }
          node = node.parentElement;
        }
  ...
  function isQuestion(el, endBoundary){
    ...
    return rect.width > 240 && rect.height > 60 && radius >= 10 &&
  ...
  function unwrapInteractive(root){
    root.querySelectorAll('[role="button"],button').forEach(el => {
      ...
      if(/avis positif|avis négatif|copier|partager|soumettre|saisie vocale|fichier/.test(label)){
        el.remove();
      } else {
  ...
  if(!endBoundary){
    return JSON.stringify({
      error:'Ni barre des pouces ni compositeur détecté',
      debug:{feedbackCount:feedbacks.length, composerFound:!!composer}
    });
  }
  ...
  if(!answerText || answerText.length < 20){
    return JSON.stringify({
      ...
      debug:{answerLength:answerText.length, boundary:lastFeedback ? 'feedback' : 'composer'}
    });
  }
  ...
  return JSON.stringify({
    ...
    debug:{
      method:'scroll-then-feedback-or-composer-range',
