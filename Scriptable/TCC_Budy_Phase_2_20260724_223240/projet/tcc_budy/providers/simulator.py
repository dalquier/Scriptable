...
class SimulatorProvider(ResponseProvider):
    ...
    def respond(self, messages) -> str:
        ...
        if any(word in lowered for word in ("peur", "angoisse", "anxieux", "panique")):
            return "Quelle pensée précise accompagne cette inquiétude, et quels faits la confirment aujourd'hui ?"
        if any(word in lowered for word in ("alcool", "cocaïne", "cocaine", "consommé", "rechute")):
            return (
                "Avant d'analyser la situation, avez-vous un symptôme physique inquiétant ou un risque immédiat ? "
                "Ensuite, nous pourrons distinguer ce qui s'est passé, les déclencheurs et le prochain pas réaliste."
            )
        return (
            "Je vous suis. Quel résultat vous aiderait le plus maintenant : être écouté, comprendre, "
            "prendre une décision ou définir une petite action ?"
        )
